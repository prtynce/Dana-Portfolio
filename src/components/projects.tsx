...
const projects: Project[] = [
  ...
  },
  {
    ...
    status: "Ongoing",
    description: "A collection of movies with trailers and ratings.",
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <p className="text-muted-foreground">
              A selection of my latest work and ongoing projects.
            </p>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project, idx) => (
            <div
              key={project.title}
              className="group glass-card rounded-3xl overflow-hidden hover:scale-[1.02] transition-all duration-500 animate-reveal"
              style={{ animationDelay: `${idx * 100}ms` }}
            >
              
              {/* DISPLAY PLACEHOLDER ONLY */}
              <div className="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-zinc-900 via-zinc-800 to-black flex items-center justify-center">

                <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.08),transparent_60%)]" />

                <div className="text-center px-6 z-10">
                  <p className="text-xs uppercase tracking-[0.3em] text-primary/60 mb-2">
                    Software Project
                  </p>
              ...
              <div className="p-8">
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[10px] font-bold uppercase tracking-wider text-primary/70 bg-primary/5 px-2 py-0.5 rounded border border-primary/10"
                    >
                      {tag}
                    </span>
                  ))}
